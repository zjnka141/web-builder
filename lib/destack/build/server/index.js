...
var file = File;
util__default["default"].inherits(File, EventEmitter$2);
...
function QuerystringParser$1(maxKeys) {
  this.maxKeys = maxKeys;
  this.buffer = '';
}
var QuerystringParser_1 = QuerystringParser$1;

QuerystringParser$1.prototype.write = function(buffer) {
  this.buffer += buffer.toString('ascii');
  return buffer.length;
};

QuerystringParser$1.prototype.end = function() {
...
util__default["default"].inherits(OctetParser$1, EventEmitter$1);

var OctetParser_1 = OctetParser$1;

OctetParser$1.prototype.write = function(buffer) {
    this.emit('data', buffer);
	return buffer.length;
};

OctetParser$1.prototype.end = function() {
	this.emit('end');
};
...
JSONParser$1.prototype.write = function(buffer) {
  this.bytesWritten += buffer.length;
  this.chunks.push(buffer);
  return buffer.length;
};

JSONParser$1.prototype.end = function() {
  try {
...
var MultipartParser = multipart_parser.MultipartParser,
    ...
    OctetParser       = octet_parser.OctetParser,
    JSONParser = json_parser.JSONParser,
    StringDecoder = require$$4__default["default"].StringDecoder,
    EventEmitter = require$$0__default["default"].EventEmitter,
    Stream = require$$6__default["default"].Stream;
...
util__default["default"].inherits(IncomingForm$1, EventEmitter);
var IncomingForm_1 = IncomingForm$1;

IncomingForm$1.prototype.parse = function(req, cb) {
  this.pause = function() {
    try {
      req.pause();
    } catch (err) {
...
IncomingForm$1.prototype.writeHeaders = function(headers) {
  this.headers = headers;
  this._parseContentLength();
  this._parseContentType();
};

IncomingForm$1.prototype.write = function(buffer) {
  if (this.error) {
    return;
  }
  if (!this._parser) {
...
IncomingForm$1.prototype.pause = function() {
  // this does nothing, unless overwritten in IncomingForm.parse
  return false;
};

IncomingForm$1.prototype.resume = function() {
  // this does nothing, unless overwritten in IncomingForm.parse
  return false;
};

IncomingForm$1.prototype.onPart = function(part) {
  // this method can be overwritten by the user
  this.handlePart(part);
};

IncomingForm$1.prototype.handlePart = function(part) {
  var self = this;
...
IncomingForm$1.prototype._parseContentType = function() {
...
IncomingForm$1.prototype._error = function(err) {
...
IncomingForm$1.prototype._parseContentLength = function() {
...
IncomingForm$1.prototype._newParser = function() {
  return new MultipartParser();
};

IncomingForm$1.prototype._initMultipart = function(boundary) {
  this.type = 'multipart';
...
IncomingForm$1.prototype._fileName = function(headerValue) {
  // matches either a quoted-string or a token (RFC 2616 section 19.5.1)
...
IncomingForm$1.prototype._initUrlencoded = function() {
...
IncomingForm$1.prototype._initOctetStream = function() {
...
IncomingForm$1.prototype._initJSONencoded = function() {
  this.type = 'json';
...
IncomingForm$1.prototype._uploadPath = function(filename) {
...
IncomingForm$1.prototype._maybeEnd = function() {
...
var IncomingForm = incoming_form.IncomingForm;
IncomingForm.IncomingForm = IncomingForm;
var lib = IncomingForm;
...
const uploadFiles = async (req) => {
  const form = new lib.IncomingForm({ uploadDir: uploadPath, keepExtensions: true });
  const uploadFolder = path__default["default"].join("public", uploadPath);
  const uploadFolderExists = await exists(uploadFolder);
  if (!uploadFolderExists) {
    await fs__default["default"].promises.mkdir(uploadFolder);
  }
  form.on("fileBegin", (_, file) => file.path = path__default["default"].join("public", uploadPath, file.name));
  const files = await formParse(form, req);
  const urls = Object.values(files).map((f) => {
    var _a;
    return path__default["default"].join(path__default["default"].sep, uploadPath, (_a = f.name) != null ? _a : "");
  });
  return urls;
};
...
const handleData = async (req, res) => {
  if (req.method === "GET") {
    const data = await loadData(req.query.id);
    ...
    if (!isMultiPart) {
      ...
      const id = body.path.split('events/').pop().replace('.json', '')
...
const publishPage = async (body) => {
  const basePath = path__default["default"].join(rootPath, folderPath);
  const fileExists = await exists(path__default["default"].join(basePath, body.path));
  if (!fileExists) {
    const folderPathExists = body.path.split(path__default["default"].sep).slice(0, -1).join(path__default["default"].sep);
    const folderExists = await exists(path__default["default"].join(basePath, folderPathExists));
    if (!folderExists) {
      await fs__default["default"].promises.mkdir(path__default["default"].join(basePath, folderPathExists), { recursive: true });
    }
    await fs__default["default"].promises.writeFile(path__default["default"].join(basePath, body.path), "{}");
  }
  await fs__default["default"].promises.writeFile(path__default["default"].join(basePath, body.path), JSON.stringify(body.data));
}
